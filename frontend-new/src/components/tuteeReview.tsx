...
export default function TuteeReview(){
    ...
    const [user, setUser] = useState<User>({
        closed: false, date: "na", displayName: "na", email: "na",
        review: "na", reviewed: true, subject:"na", tutor:"na"
    })
    useEffect(() => {getUserDocs(queryParameters.get("request")).then((user)=>{setUser(user)})}, []);
    function saveReview(){
    ...
    return (<div className="App-bg">
        <p className="prof-comp">review tutor {user.tutor} ?

            <div>
                <input placeholder="Enter Review" className="txt-Box2" value={review}
                       onChange={(e) => setReview(e.target.value)}/>
                <button className="conf-button" onClick={saveReview}> Confirm</button>
            </div>
        </p>
    </div>)
}